...
public class GridView : MonoBehaviour
{
    ...
    /// </summary>
    public void ShowPreview(CardData card, int originX, int originY, int rotationSteps = 0)
    {
        RefreshGrid(); // 기존 상태 먼저 복원

        if (card == null) return;

        bool canPlace = gridManager.CanPlaceBlock(card, originX, originY, rotationSteps);
